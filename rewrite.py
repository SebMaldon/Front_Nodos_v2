...
render_end = content.rfind('};', render_start)
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden scale-in" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex justify-end gap-3 pt-4 border-t">
                                    ...
                                    <Button onClick={() => setShowMaterialesModal(false)} className="bg-imss-green hover:bg-imss-green/90 text-white">Guardar Selección</Button>
                                </div>
                            </div>
                        </div>
                    </div>
...
# Need to replace the custom TextField render function as well because we removed TextField import
render_cantidad = content.find('const renderCantidadInput')
if render_cantidad != -1:
    end_render_cantidad = content.find('};', render_cantidad) + 2
    # we don't need this render function anymore, we inlined it in the modal
    content = content[:render_cantidad] + content[end_render_cantidad:]
